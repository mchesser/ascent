...
pub(crate) fn compile_mir(mir: &AscentMir, is_ascent_run: bool) -> proc_macro2::TokenStream {
   ...
   let extra_fields = match &mir.declaration.fields {
      Some(fields) => {
   ...
   let init_fn = match &mir.declaration.fields {
      None => quote! {
      ...
      Some(fields) => {
         let fields_with_types = &fields.named;
         let fields = fields.named.iter().map(|x| x.ident.as_ref().unwrap());
         quote! {
            impl #impl_generics #struct_name #ty_generics #where_clause {
               pub fn new(#fields_with_types) -> Self {
                  let mut _self = #struct_name {
                     #(#fields,)*
                     #(#field_defaults)*
                     #(#scc_time_field_defaults)*
...
fn scc_fn_name(i: usize) -> Ident {
   Ident::new(&format!("scc{}", i), Span::call_site())
}
...
fn head_clauses_structs_and_update_code(rule: &MirRule, scc: &MirScc, mir: &AscentMir) -> (proc_macro2::TokenStream, proc_macro2::TokenStream) {
   ...
   for hcl in rule.head_clause.iter() {
      ...
      let all_used = (0..hcl.rel.field_types.len()).all(|i| used_fields.contains(&i));

      let new_row_to_be_pushed = if all_used {
         syn::parse2(quote_spanned!{hcl.span=> __new_row.clone()}).unwrap()
      }
      else { 
         ...
         tuple_spanned(&new_row_to_be_pushed, hcl.span)
      };
